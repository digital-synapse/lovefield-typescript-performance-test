define(["require", "exports", 'lovefield/dist/lovefield'], function (require, exports) {
    function sequence() {
        var items = [];
        ...
        if (d) {
            ...
            var args = d;
            fn(args).then(function () { sequence.apply(void 0, items); });
        }
    }
    (function () {
        ...
        var dummyItem = null;
        var inserts = 50000;
        sequence([testSelectPredicate, 'INDEXED_DB'], [testSelectPredicate, 'WEB_SQL'], [testSelect, 'INDEXED_DB'], [testSelect, 'WEB_SQL'], [testInsert, 'INDEXED_DB'], [testInsert, 'WEB_SQL'], [dropTable, 'INDEXED_DB'], [dropTable, 'WEB_SQL'], [testInsertBatch, 'INDEXED_DB'], [testInsertBatch, 'WEB_SQL'], [dropTable, 'INDEXED_DB'], [dropTable, 'WEB_SQL']);
        function dropTable(storeType) {
            connectOptions = {
                storeType: lf.schema.DataStoreType[storeType]
            };
            var starttime = Date.now();
            return schemaBuilder.connect(connectOptions).then(function (db) {
        ...
        function testInsertBatch(storeType) {
            return new Promise(function (resolve, reject) {
                connectOptions = {
                    storeType: lf.schema.DataStoreType[storeType]
                };
        ...
        function testSelectPredicate(storeType) {
            return new Promise(function (resolve, reject) {
                connectOptions = {
                    storeType: lf.schema.DataStoreType[storeType]
                };
        ...
        function testSelect(storeType) {
            return new Promise(function (resolve, reject) {
                ...
                schemaBuilder.connect(connectOptions).then(function (db) {
                    ...
                    if (!rows.length || rows.length < inserts)
                        console.error("lovefield:" + storeType + " did not select the expected number of rows!");
                    else {
                        var duration = Date.now() - starttime;
